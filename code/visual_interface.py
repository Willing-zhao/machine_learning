...
class UI(object):
    ...
    def setup_ui(self, form):
        ...
        self.label_designer.setGeometry(QtCore.QRect(20, 700, 280, 40))
        font = QtGui.QFont()
        ...
        self.label_bar.setGeometry(QtCore.QRect(680, 150, 200, 180))
    ...
    def retranslate_ui(self, form):
        ...
        form.setWindowTitle(_translate("emotion_reco", "emotion_reco"))
        self.label_raw_pic.setText(_translate("emotion_reco", "To be entered"))
        self.label_designer.setText(_translate("emotion_reco", "成员：赵卫星 黄玮翔 刘玉刚"))
        self.pushButton_select_img.setText(_translate("emotion_reco", "select"))
        self.label_result.setText(_translate("emotion_reco", "result"))
        self.label_emotion.setText(_translate("emotion_reco", "null"))
        self.label_bar.setText(_translate("emotion_reco", "Probability histogram"))
        self.label_rst.setText(_translate("emotion_reco", "Result"))

    def open_file_browser(self):
        # 加载模型
        file_name, file_type = QtWidgets.QFileDialog.getOpenFileName(caption="选取图片", directory="../input/picture/",
    ...
    def show_results(self, emotion, possibility):
        # 显示表情名
        self.label_emotion.setText(QtCore.QCoreApplication.translate("emotion_reco", emotion))
        # 显示emoji
        if emotion != 'no':
            img = cv2.imread('../resource/icons/' + str(emotion) + '.png')
            frame = cv2.resize(cv2.cvtColor(img, cv2.COLOR_BGR2RGB), (100, 100))
            self.label_rst.setPixmap(QtGui.QPixmap.fromImage(
                QtGui.QImage(frame.data, frame.shape[1], frame.shape[0], 3 * frame.shape[1],
                             QtGui.QImage.Format_RGB888)))
        else:
            self.label_rst.setText(QtCore.QCoreApplication.translate("emotion_reco", "no result"))
        # 显示直方图
    ...
    def get_faces_from_image(self, img_path):
        ...
        """
        import cv2
        face_cascade = cv2.CascadeClassifier('../DSN/params/haarcascade_frontalface_alt.xml')
