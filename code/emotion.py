...
def get_fer2013_images():
    ...
    save_image_from_fer2013('../DSN/fer2013/fer2013.csv')
...
def get_jaffe_images():
    ...
    def detect_face(img):
        cascade = cv2.CascadeClassifier('../DSN/params/haarcascade_frontalface_alt.xml')
    ...
    for file in files:
        ...
        index += 1
    if not os.path.exists('../DSN/jaffe/Training'):
        os.mkdir('../DSN/jaffe/Training')
    for i in range(len(images)):
        path_emotion = '../DSN/jaffe/Training/{}'.format(emotions_reverse[labels[i]])
        if not os.path.exists(path_emotion):
            os.mkdir(path_emotion)
        cv2.imwrite(os.path.join(path_emotion, '{}.jpg'.format(i)), images[i])
    print("load jaffe DSN")
...
def cv2_img_add_text(img, text, left, top, text_color=(0, 255, 0), text_size=20):
    ...
    font_text = ImageFont.truetype(
        "../resource/simsun.ttc", text_size, encoding="utf-8")  # 使用宋体
