...
parser.add_argument("--DSN", type=str, default="fer2013", help="DSN to train, fer2013 or jaffe or ck+")
...
if opt.dataset == "fer2013":
    ...
    print("load fer2013 DSN successfully, it has {} train images and {} valid iamges".format(y_train.shape[0], y_valid.shape[0]))
    ...
    print("picture accuacy", np.sum(pred.reshape(-1) == y_test.reshape(-1)) / y_test.shape[0])
    ...
    print("load jaffe DSN successfully, it has {} train images and {} valid iamges".format(y_train.shape[0],
                                                                                                 y_valid.shape[0]))
    ...
    # 划分训练集验证集
    x_train, x_valid, y_train, y_valid = train_test_split(x, y, test_size=0.2, random_state=2019)
    print("load CK+ DSN successfully, it has {} train images and {} valid iamges".format(y_train.shape[0],
                                                                                y_valid.shape[0]))
    train_generator = ImageDataGenerator(rotation_range=10,
                                         width_shift_range=0.05,
                                         height_shift_range=0.05,
